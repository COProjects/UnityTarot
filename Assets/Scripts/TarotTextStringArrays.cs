...
public class TarotTextStringArrays : MonoBehaviour {
	private string[] celticPositions={"Present","Challenge","Foundation","Recent past","Best outcome","Immediate Future","Inner Feelings","External influences","Hopes or fears","Final outcome"};
	...
	private string[,] cups = new string[14,3] {
		...
		{"Twoof Cups", "A new romance, a well balanced friendship is beginning, harmony, cooperation" , " A loss of balance in a relationship, a violent passion, love turning bad, a misunderstanding"} , 		
	...
	void Awake () {
		int Rone = Random.Range (0, 13);
			int Rtwo=Random.Range(1,2);
		string myFortune = pentacles[Rone,0] +"\n"+ pentacles[Rone,Rtwo];
		print (myFortune);
		 myFortune = wands[Rone,0] +"\n"+ wands[Rone,Rtwo];
		print (myFortune);
		 myFortune = cups[Rone,0] +"\n"+ cups[Rone,Rtwo];
		print (myFortune);
		 myFortune = swords[Rone,0] +"\n"+ swords[Rone,Rtwo];
		print (myFortune);
}
